...
abstract contract V3SwapRouter is IV3SwapRouter, PeripheryPaymentsWithFeeExtended, OracleSlippage {
    ...
    function exactInputSingle(
        ...
        emit V3Swap(params.tokenIn, params.tokenOut, params.amountIn, amountOut, params.recipient);
    }
    ...
    function exactInput(ExactInputParams memory params) external payable override returns (uint256 amountOut) {
        ...
        require(amountOut >= params.amountOutMinimum, 'Too little received');
        emit V3Swap(tokenIn, tokenOut, params.amountIn, amountOut, params.recipient);
    }
    ...
    function exactOutputSingle(
        ...
        emit V3Swap(params.tokenIn, params.tokenOut, amountIn, params.amountOut, params.recipient);
    }

    /// @inheritdoc IV3SwapRouter
    function exactOutput(ExactOutputParams memory params) external payable override returns (uint256 amountIn) {
        exactOutputInternal(
            params.amountOut,
            params.recipient,
            0,
        ...
        address tokenIn;
        address tokenOut;
        (tokenIn, , ) = params.path.decodeFirstPool();
        while (params.path.hasMultiplePools()) {
            params.path = params.path.skipToken();
        }
        (, tokenOut, ) = params.path.decodeFirstPool();
        emit V3Swap(tokenIn, tokenOut, amountIn, params.amountOut, params.recipient);
    }
}
